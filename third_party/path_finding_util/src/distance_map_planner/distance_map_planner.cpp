...
std::vector<bool> DMPlanner<Dim>::setPath(const vec_Vecf<Dim> &path,
  ...
  auto dim = map_util_->getLocalMapNumVoxels();
...
template <int Dim>
std::shared_ptr<GridMap> DMPlanner<Dim>::getMapUtil() {
  return map_util_;
}
...
template <int Dim> vec_Vec3f DMPlanner<Dim>::getCloud(double h_max) {
  ...
  //       // if (data[idx] > 0) {
  //       double occ_val = map_util_->getOccVal(n);
  //       if (occ_val > 0.5) {
  //         double h = occ_val * h_max / H_MAX;
...
template <int Dim>
void DMPlanner<Dim>::setMap(const std::shared_ptr<GridMap> map_util,
                            const Vecf<Dim> &pos) {
  map_util_ = map_util;
