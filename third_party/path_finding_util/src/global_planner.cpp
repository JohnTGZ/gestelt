...
std::vector<std::vector<Eigen::Vector3d>> GlobalPlanner::PlanPRM(
    ...
    const std::shared_ptr<GridMap> vg,
...
std::vector<Eigen::Vector3d> GlobalPlanner::PlanAStar(
    const Eigen::Vector3d &start, const Eigen::Vector3d &goal,
    const std::shared_ptr<GridMap> vg, const int max_expand,
  ...
  GraphSearch graph_search(vg->getGlobalMapNumVoxels(), weight_heur, verbose);
...
std::vector<Eigen::Vector3d> GlobalPlanner::PlanAStarAPF(
    const Eigen::Vector3d &start, const Eigen::Vector3d &goal,
    const std::shared_ptr<GridMap> vg, const double apf_coeff,
  ...
  GraphSearch graph_search(vg->getGlobalMapNumVoxels(), 1, false);
