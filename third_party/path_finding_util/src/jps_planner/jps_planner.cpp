...
template <int Dim>
void JPSPlanner<Dim>::setMapUtil(const std::shared_ptr<GridMap> map_util) {
  map_util_ = map_util;
}
...
void JPSPlanner<Dim>::updateMap() {
  Veci<Dim> dim = map_util_->getLocalMapNumVoxels();
...
bool JPSPlanner<Dim>::plan(const Vecf<Dim> &start, const Vecf<Dim> &goal, decimal_t eps, bool use_jps) {
  if(planner_verbose_){
    ...
    std::cout <<"Goal_int:  " << goal_int.transpose() << std::endl;
  }
  ...
  if (!map_util_->isFree(start_int)) {
    if(planner_verbose_) {
      if (map_util_->isOccupied(start_int)){
        ...
        std::cout <<"Map dim: " << map_util_->getLocalMapNumVoxels().transpose() << std::endl;
      }
    }
    status_ = 1;
    return false;
  ...
  const Veci<Dim> dim = map_util_->getLocalMapNumVoxels();

  std::cout << "Start planning" << std::endl;

  auto a = std::chrono::high_resolution_clock::now();

  graph_search_ = std::make_shared<JPS::GraphSearch>(map_util_, dim(0), dim(1), dim(2), eps, planner_verbose_);
  ...
  std::cout << "Planning duration: "<< plan_dur << " s" << std::endl;
