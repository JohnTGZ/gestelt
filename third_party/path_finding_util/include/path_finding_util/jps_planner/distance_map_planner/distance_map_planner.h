...
#include <path_finding_util/jps_basis/data_type.h>
...
public:
  ...
  std::shared_ptr<GridMap> getMapUtil();
   ...
   * the internal map.
   */
  void setMap(const std::shared_ptr<GridMap> map_util,
              const Vecf<Dim>& pos);
...
protected:
  ...
  std::shared_ptr<GridMap> map_util_;
