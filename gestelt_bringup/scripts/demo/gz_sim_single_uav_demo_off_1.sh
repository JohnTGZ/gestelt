...
#####
CMD_0="
roslaunch --wait gestelt_bringup single_uav_sim.launch drone_id:=1 init_x:=0.5 init_y:=-0.5
"

CMD_1="roslaunch --wait gestelt_bringup single_fake_map.launch drone_id:=1"

CMD_2="
roslaunch --wait gestelt_bringup single_ego_planner.launch drone_id:=1 init_x:=0.5 init_y:=-0.5 POSE_TYPE:=3 SENSOR_TYPE:=1
"
