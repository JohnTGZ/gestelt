...
public:
  ...
  bool stop(bool print_dur = false) {
    ...
    t_dur_cpu_ = std::chrono::duration_cast<std::chrono::duration<double>>(
            std::chrono::high_resolution_clock::now() - t_start_cpu_).count() * 1000.0;
    t_dur_wall_ = std::chrono::duration_cast<std::chrono::duration<double>>(
            std::chrono::system_clock::now() - t_start_wall_).count() * 1000.0;

    timer_running_ = false;

    t_cum_dur_cpu_ += t_dur_cpu_;
    t_cum_dur_wall_ += t_dur_wall_;

    if (print_dur) {
      printf(ANSI_COLOR_MAGENTA "%sTimer[%s](wall): %f ms \n" ANSI_COLOR_RESET, 
            indent_str_.c_str(), name_.c_str(), t_dur_wall_);
...
private:
  ...
  double t_dur_cpu_{0.0};     // Current CPU time duration in ms
  double t_dur_wall_{0.0};    // Current wall time duration in ms
